...
class ConnectFour:
    ...
    def __init__(self, height=6, width=7):
        ...
        self.white = f"{Fore.YELLOW}●{Fore.RESET}"
        self.black = f"{Fore.RED}○{Fore.RESET}"
    ...
    def print_lines(self, *lines):
        ...
        """
        for line in lines:
            print(line.center(TERMX).replace("●", self.white).replace("○", self.black))
    ...
    def print_board(self):
        ...
        self.print_lines(header, *gutter, footer)
    ...
    def is_valid_move(self, move: str) -> bool:
        ...
        if not (len(move) == 1 and move in self.labels):
            self.print_lines("Please input a valid column!")
            return False
        ...
        self.print_lines("No moves possible in that column!")
        return False

    def get_valid_move(self) -> str:
        player = "●○"[self.current_player - 1]
        while True:
            self.print_lines(f"{player}'s move, enter column or 'q' to quit:\n")
    ...
    def run(self):
        for _ in range(self.width * self.height):
            ...
            if self.is_winner(player):
                self.print_board()
                self.print_lines(f"{'●○'[player - 1]} wins!".replace("●", self.white).replace("○", self.black))
                break

        else:
            self.print_board()
            self.print_lines("It's a draw!")
